...
async function capture(
  ...
  entry.capture = (async () => {
    ...
    // The handoff file was written before the continuation WAL record. It becomes visible to
    // retries only after that WAL transition is durable; a failed state write therefore keeps
    // the transaction in awaiting-summary and lets the same semantic step be retried.
