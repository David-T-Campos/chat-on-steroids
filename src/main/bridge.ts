...
function commandSnapshot(options: {
  ...
  return { version: 2, commands: records, receipts };
}
...
export async function restoreCommands(): Promise<void> {
  ...
  if (!saved || (saved.version !== 1 && saved.version !== 2) || !Array.isArray(saved.commands)) return;
  const now = Date.now();
  if (saved.version === 2 && Array.isArray(saved.receipts)) {
  ...
  for (const raw of saved.commands as Array<Partial<DurableCommandRecord>>) {
    ...
    if (
      ...
      spec = { type: 'worker', agent: worker.agent, task: worker.task.slice(0, 512 * 1024) };
    } else if (
      specRaw.type === 'resume' &&
    ...
    const persistedLeased = saved.version === 2 && raw.phase === 'leased';
