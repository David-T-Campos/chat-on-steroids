  ...
  reg.register(
    ...
    async (input) =>
      guard('session', async () => {
        ...
        if (recentOnly) {
          events = await readRecentEvents(id, cap, input.kind ? { kinds: [input.kind] } : {});
        } else if (input.call_id) {
          // Expanding a call is overwhelmingly a follow-up to something the recent history
          // just showed. Avoid reparsing a years-long journal for that common case, while
          // preserving exact +/-3 context by falling back whenever the left neighborhood may
          // cross the bounded tail. The tail's right edge is authoritative because it ends at
          // the newest event.
          const recent = await readRecentEvents(id, 64);
          const recentIndex = recent.findIndex(
            (event) => event.kind === 'tool_call' && event.call.callId === input.call_id
          );
          events =
            recentIndex >= 0 && (recentIndex >= 3 || recent.length < 64)
              ? recent
              : await readEvents(id, {});
        } else {
          events = await readEvents(id, input.kind ? { kinds: [input.kind] } : {});
        }
      ...
      return guard('agents', async () => {
        ...
        if (input.action === 'spawn') {
          ...
          if (!(await persistCriticalSwarmNow())) {
            throw new Error('The worker run could not cross its durable acceptance barrier. Retry this same spawn request.');
          }
        ...
        if (input.action === 'message') {
          ...
          const sent = sendMessages(await callerNow(startedAt), items);
          if (!(await persistCriticalSwarmNow())) {
            throw new Error('The agent message was not durably accepted yet. The broker kept its state in memory; do not assume delivery.');
          }
...
async function expandGlob(
  ...
  const candidates = rest.includes('**')
    ? (await walkFiles(resolved.real, resolved.virtual, { maxEntries: GLOB_SCAN_LIMIT, exclude: DEFAULT_EXCLUDES }))
        .files
    : (await listDirectoryLevel(resolved.real, resolved.virtual, GLOB_SCAN_LIMIT, false)).entries
